...
class MemberTransferView(APIView):
    ...
    def post(self, request):
        ...
        if to_altar:
            ...
            member.save()
            message = (
                f"Member {member.full_name} has been deactivated and "
                f"offboarded from {from_altar.name}"
            )
...
class BulkAttendanceView(APIView):
    ...
    def post(self, request):
        ...
        if not request.user.is_superuser and not request.user.can_manage_unit(altar):
            return Response({
                "error": (
                    f"You don't have permission to record attendance for '{altar.name}'. "
                    f"You can only record attendance for altars within your organizational scope."
                )
            }, status=status.HTTP_403_FORBIDDEN)
        ...
        if not all([sub_region, region, country, continent]):
            missing_levels = [
                level for level in ["CONTINENT", "COUNTRY", "REGION", "SUB_REGION"]
                if level not in hierarchy
            ]
            return Response(
                {
                    'error': (
                        f'Incomplete organizational hierarchy for altar "{altar.name}". '
